...
sudo systemctl <start|stop|restart|status> portfolio-puma.service
sudo systemctl <start|stop|restart|status> portfolio-sidekiq.service
```

Note that the `deployer` user has permission to restart `portfolio-puma.service` and `portfolio-sidekiq.service`, but nothing else. All other `systemctl` commands require `root`.
...
```
/home/deployer/apps/portfolio/shared/.env.production
```

After making changes to this file, be sure to restart the application:

```
sudo systemctl restart portfolio-puma.service
sudo systemctl restart portfolio-sidekiq.service
```
