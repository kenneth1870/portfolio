# How to provision a new environment for portfolio
...
* Enter domains to secure (for example): `kennethrosales`
* Enter webroot path: `/home/deployer/apps/portfolio/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/kennethrosales/fullchain.pem /etc/ssl/portfolio.crt
ln -s /etc/letsencrypt/live/kennethrosales/privkey.pem /etc/ssl/portfolio.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/portfolio/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
32 4,16 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/portfolio/shared/public/
```
